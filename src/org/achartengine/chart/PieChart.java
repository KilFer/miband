...
public class PieChart extends RoundChart
{
  ...
  public void draw(Canvas paramCanvas, int paramInt1, int paramInt2, int paramInt3, int paramInt4, Paint paramPaint)
  {
    ...
    int m = 0;
    double d3;
    for (double d1 = 0.0D; m < k; d1 = d3)
    {
      d3 = d1 + this.mDataset.getValue(m);
      arrayOfString[m] = this.mDataset.getCategory(m);
      m++;
    }
    if (this.mRenderer.isFitLegend());
    for (int n = drawLegend(paramCanvas, this.mRenderer, arrayOfString, paramInt1, j, paramInt2, paramInt3, paramInt4, i, paramPaint, true); ; n = i)
    {
      ...
      int i4;
      label378: SimpleSeriesRenderer localSimpleSeriesRenderer;
      label442: float f4;
      float f5;
      if (!this.a.areAllSegmentPresent(k))
      {
        ...
        if (i4 >= k)
          break label799;
        localSimpleSeriesRenderer = this.mRenderer.getSeriesRendererAt(i4);
        ...
        float f8 = (float)(0.1D * i2 * Math.sin(d2));
        float f9 = (float)(0.1D * i2 * Math.cos(d2));
      ...
      while (true)
      {
        ...
        i3 = 0;
        break;
        label770: paramPaint.setColor(localSimpleSeriesRenderer.getColor());
        break label442;
        label783: paramCanvas.drawArc(localRectF, f1, f5, true, paramPaint);
      }
      label799: localArrayList.clear();
